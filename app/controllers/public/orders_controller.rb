  ...
  class OrdersController < ApplicationController
    ...
    def create
      order = Order.new user_id: current_user.id,
        date_order: Time.zone.today, total_price: session[:total_price]
      order.save
      order = Order.where("user_id = ?", current_user.id).first
      session[:order_items].each do |row|
        product = Product.find_by id: row[1]
        order_detail = OrderDetail.new product_id: row[1],
           quantity: row[0], order_id: order.id, price: product.price
        order_detail.save
      end
